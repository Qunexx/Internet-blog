    ...
    <div class="center-both">
        ...
        <form action="{{ route('post.update',$post->id)}}" method="post">
            ...
            <div class="mb-3">
                ...
                <textarea value="{{old('post_content')}}" name="post_content" class="form-control" id="content"
                          value="{{$post->post_content }}" placeholder="post_content"></textarea>
                @error('post_content')
                <p class="text-danger">{{$message}}</p>
                @enderror
            </div>
            <div class="mb-3">
                ...
                <input value="{{old('image')}}" type="text" name="image" class="form-control" id="image"
                       value="{{$post->image}}" placeholder="image">
                @error('image')
                <p class="text-danger">{{$message}}</p>
                @enderror
            </div>
